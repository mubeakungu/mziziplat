    ...
    <div className="space-y-8">
        ...
        <div className="relative z-10 flex items-center justify-between">
          <div>
            ...
            <h1 className="text-5xl md:text-6xl font-bold mb-3 text-white font-heading tracking-tight">
              <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-400 via-amber-200 to-blue-400 animate-text-shimmer bg-[length:200%_auto]">Transaction History</span>
            </h1>
            <p className="text-gray-400 text-lg">Complete record of all your financial activities</p>
          </div>
          <button className="px-6 py-3 bg-gradient-to-r from-blue-500 to-amber-600 rounded-xl font-bold text-white hover:shadow-[0_0_30px_rgba(59,130,246,0.3)] transition-all flex items-center gap-2">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="relative bg-black/60 border border-white/10 backdrop-blur-xl rounded-2xl p-6 hover:border-white/20 transition-all">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-2xl font-bold text-green-400 font-heading">Ksh {totalDeposits.toFixed(2)}</p>
              </div>
            </div>
          </div>
        </motion.div>
          ...
          <div className="relative bg-black/60 border border-white/10 backdrop-blur-xl rounded-2xl p-6 hover:border-white/20 transition-all">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-2xl font-bold text-red-400 font-heading">Ksh {totalWithdrawals.toFixed(2)}</p>
              </div>
            </div>
          </div>
        </motion.div>
        ...
        >
          ...
          <div className="relative bg-black/60 border border-white/10 backdrop-blur-xl rounded-2xl p-6 hover:border-white/20 transition-all">
            <div className="flex items-center gap-4">
              <div className="w-12 h-12 rounded-xl bg-gradient-to-br from-amber-500/20 to-pink-500/20 border border-white/10 flex items-center justify-center">
                <DollarSign className="w-6 h-6 text-amber-400" />
              </div>
              <div>
                <p className="text-sm text-gray-400 uppercase tracking-wider">Total Wagered</p>
                <p className="text-2xl font-bold text-amber-400 font-heading">Ksh {totalWagered.toFixed(2)}</p>
              </div>
            </div>
          </div>
        </motion.div>
            ...
            {(['ALL', 'DEPOSIT', 'WITHDRAWAL', 'BET', 'WIN'] as const).map((type) => (
              <button
                ...
                className={`px-5 py-2.5 rounded-xl font-bold transition-all hover:scale-105 ${
                  filter === type
                    ? 'bg-gradient-to-r from-primary to-amber-600 text-white shadow-[0_0_20px_rgba(102,126,234,0.3)]'
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <div className="text-right">
                      <p className={`text-2xl font-bold font-heading ${getTypeColor(tx.type)}`}>
                        {['DEPOSIT', 'WIN', 'BONUS'].includes(tx.type) ? '+' : '-'}Ksh {Math.abs(tx.amount).toFixed(2)}
                      </p>
                      <p className="text-xs text-gray-500 mt-1">
                        Balance: <span className="text-gray-400 font-medium">Ksh {tx.balanceAfter.toFixed(2)}</span>
                      </p>
                    </div>
                  </div>
                </motion.div>
