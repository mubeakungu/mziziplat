...
const VIP_LEVELS = {
  ...
  PLATINUM: { color: 'text-amber-400', bg: 'bg-amber-500/10', next: 'DIAMOND', threshold: 1000000 },
          ...
          <div className="space-y-3">
            ...
            <div className="flex items-center justify-between pt-2 border-t border-white/10">
              <div>
                ...
                <p className="text-white font-bold">Ksh {(vipData?.totalWagered || 0).toLocaleString()}</p>
              </div>
              {levelConfig.next && (
                <div className="text-right">
                  <p className="text-xs text-gray-500">Next Level</p>
                  <p className="text-white font-bold">Ksh {vipData?.nextLevelWager.toLocaleString()}</p>
                </div>
              )}
            </div>
            ...
            {levelConfig.next && (
              <div className="flex items-center gap-2 text-xs text-gray-400 bg-black/30 p-2 rounded">
                ...
                <span>Ksh {((vipData?.nextLevelWager || 0) - (vipData?.totalWagered || 0)).toLocaleString()} to {levelConfig.next}</span>
              </div>
            )}
          </div>
        </CardContent>
