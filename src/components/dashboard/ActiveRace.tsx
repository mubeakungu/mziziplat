...
export function ActiveRace() {
  ...
  return (
    <Card className="bg-gradient-to-br from-emerald-900/40 to-amber-900/40 border-emerald-500/30 backdrop-blur w-full overflow-hidden relative group">
      <div className="absolute inset-0 bg-[url('/grid.svg')] opacity-20" />
      <div className="absolute -right-10 -top-10 w-32 h-32 bg-emerald-500/20 rounded-full blur-3xl animate-pulse" />
      ...
      <CardHeader className="pb-2 relative z-10">
        ...
        <p className="text-xs text-emerald-200/70 font-medium tracking-wide">
          Prize Pool: <span className="text-white font-bold">Ksh {data.race.prizePool.toLocaleString()}</span>
        </p>
      </CardHeader>
      ...
      <CardContent className="space-y-3 relative z-10">
        ...
        <div className="space-y-2 max-h-[240px] overflow-y-auto pr-1 scrollbar-thin scrollbar-thumb-emerald-500/20">
            ...
            {data.leaderboard?.slice(0, 5).map((entry, index) => (
              ...
              >
                ...
                <Avatar className="w-8 h-8 rounded-lg border border-white/10">
                  ...
                  <AvatarFallback className="bg-emerald-950 text-emerald-200 text-xs">
                    {entry.user.name?.[0]}
                  </AvatarFallback>
                </Avatar>
                ...
                <div className="text-right">
                  <p className="text-sm font-mono font-bold text-white">
                    Ksh {(entry.totalWagered / 1000).toFixed(1)}k
                  </p>
                </div>
              </motion.div>
            ))}
          ...
          {(!data.leaderboard || data.leaderboard.length === 0) && (
            <div className="text-center py-4">
              <p className="text-sm text-emerald-300/50">Race just started! bet to join.</p>
            </div>
          )}
        </div>
        ...
        <motion.button
          ...
          whileTap={{ scale: 0.98 }}
          className="w-full py-2 bg-gradient-to-r from-emerald-600 to-amber-600 hover:from-emerald-500 hover:to-amber-500 text-white text-xs font-bold uppercase tracking-wider rounded-lg shadow-lg shadow-emerald-500/20 transition-all border border-white/10"
        >
          Join Race
        </motion.button>
      </CardContent>
