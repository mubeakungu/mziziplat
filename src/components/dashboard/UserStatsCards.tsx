...
export function UserStatsCards() {
      ...
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-4">
        {[...Array(4)].map((_, i) => (
          <div key={i} className="relative group">
            <div className="absolute -inset-0.5 bg-gradient-to-r from-primary to-amber-600 rounded-2xl blur opacity-20 animate-pulse" />
  ...
  const cards = [
    {
      title: 'Total Balance',
      value: `Ksh ${((stats?.mainBalance || 0) + (stats?.bonusBalance || 0)).toFixed(2)}`,
      subtitle: `Main: Ksh ${(stats?.mainBalance || 0).toFixed(2)} | Bonus: Ksh ${(stats?.bonusBalance || 0).toFixed(2)}`,
      icon: Wallet,
      color: 'text-cyan-400',
    ...
    {
      title: 'Total Wagered',
      value: `Ksh ${(stats?.totalWagered || 0).toFixed(2)}`,
      icon: Target,
      color: 'text-amber-400',
      glowColor: 'from-amber-500/20 to-pink-500/20',
      borderGlow: 'group-hover:shadow-[0_0_30px_rgba(168,85,247,0.3)]',
    },
    {
      title: 'Total Won',
      value: `Ksh ${(stats?.totalWon || 0).toFixed(2)}`,
      icon: Trophy,
    ...
    {
      title: 'Net Profit',
      value: `Ksh ${(stats?.netProfit || 0).toFixed(2)}`,
      icon: TrendingUp,
