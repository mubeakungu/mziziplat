  ...
  return (
    <div className="flex flex-col items-center justify-center p-4">
      <Card className="w-full max-w-md glass-card border-white/10 bg-black/40">
        ...
        <CardContent className="space-y-8">
          <div className="flex justify-center gap-4 py-8 bg-black/50 rounded-xl border border-white/10 shadow-inner relative overflow-hidden">
            ...
            {reels.map((symbol, i) => (
              <motion.div
                key={i}
                className="w-20 h-24 bg-linear-to-b from-gray-800 to-black border-2 border-amber-500/30 rounded-lg flex items-center justify-center text-4xl shadow-[0_0_15px_rgba(168,85,247,0.2)] z-10"
                animate={isSpinning ? {
                  y: [0, -20, 0],
                  filter: "blur(2px)",
                  scale: 0.95
          ...
          <div className="space-y-2 bg-black/30 p-4 rounded-lg border border-white/5">
            ...
            <div className="relative">
              ...
              <Input
                ...
                disabled={isSpinning}
                min={1}
                className="pl-8 bg-black/50 border-white/10 focus:border-amber-500/50 text-white"
              />
            </div>
          </div>
        ...
        <CardFooter>
          <Button 
            className="w-full h-12 text-lg font-bold bg-amber-600 hover:bg-amber-700 text-white shadow-[0_0_20px_rgba(147,51,234,0.3)] border-none"
            onClick={spinReels} 
            disabled={isSpinning}
          >
