...
export class FraudDetectionService {
  ...
  static async checkWithdrawalPattern(userId: string, amount: number): Promise<{ flagged: boolean; reason?: string }> {
    ...
    if (accountAgeHours < 24 && amount + totalWithdrawals > totalDeposits + 1000) {
      await this.flagUser(userId, 'SUSPICIOUS_WITHDRAWAL', `New account withdrawing Ksh ${amount}`)
      return {
        flagged: true,
    ...
    if (recentDeposit && amount > recentDeposit.amount * 0.9) {
      await this.flagUser(userId, 'RAPID_WITHDRAWAL', `Withdrawing Ksh ${amount} after Ksh ${recentDeposit.amount} deposit`)
      return {
        flagged: true,
