...
export class NotificationService {
  ...
  static async notifyDeposit(userId: string, amount: number) {
    return this.create(
      ...
      'Deposit Successful',
      `Ksh ${amount} has been added to your wallet`,
      { amount }
    )
  }
  ...
  static async notifyWithdrawal(
    ...
    const messages = {
      ...
      COMPLETED: `Ksh ${amount} has been transferred to your account`,
  ...
  static async notifyBigWin(userId: string, game: string, amount: number, multiplier: number) {
    if (amount >= 1000) { // Only notify for big wins
      return this.create(
        ...
        'Big Win! 🎉',
        `You won Ksh ${amount} with ${multiplier}x multiplier on ${game}!`,
  ...
  static async notifyBonus(userId: string, bonusType: string, amount: number) {
    return this.create(
      ...
      'Bonus Received! 🎁',
      `You received Ksh ${amount} ${bonusType} bonus`,
      { bonusType, amount }
    )
  }
