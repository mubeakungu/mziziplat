...
export class TransactionLimitService {
  ...
  static async checkDailyWithdrawalLimit(userId: string, amount: number): Promise<{ allowed: boolean; remaining: number; message?: string }> {
    ...
    if (newTotal > dailyMax) {
      return {
        ...
        message: `Daily withdrawal limit exceeded. You can withdraw Ksh ${(dailyMax - todayTotal).toFixed(2)} more today.`,
      }
    }
  ...
  static async validateTransaction(
    ...
    if (amount < typeLimits.min) {
      return {
        valid: false,
        error: `Minimum ${type} amount is Ksh ${typeLimits.min}`,
      }
    }
    ...
    if (amount > typeLimits.max) {
      return {
        valid: false,
        error: `Maximum ${type} amount is Ksh ${typeLimits.max}${limits.kycApproved ? '' : '. Complete KYC to increase limits.'}`,
      }
    }
